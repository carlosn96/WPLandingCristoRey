...
<div class="wp-block-woocommerce-empty-cart-block">
	...
	<div class="wp-block-group"
		...
		<h2 class="wp-block-heading has-text-align-center has-large-font-size">
			<?php esc_html_e('Your shopping bag is empty', 'assembler'); ?></h2>
		<!-- /wp:heading -->
		...
		<div class="wp-block-buttons">
			...
			<div class="wp-block-button"><a
					class="wp-block-button__link has-small-font-size has-custom-font-size wp-element-button"
					href="<?php echo esc_url(home_url('/shop')); ?>"><?php esc_html_e('Return to shop', 'assembler'); ?></a>
			</div>
			<!-- /wp:button -->
		</div>
		<!-- /wp:buttons -->
		...
		<p class="has-text-align-center has-small-font-size" style="margin-top:var(--wp--preset--spacing--30)">
			<?php /* Translators: 1. is the start of a 'a' HTML element, 2. is the end of a 'a' HTML element */
			echo sprintf(esc_html__('Have an account? %1$sLog in%2$s to check out faster.', 'assembler'), '<a href="' . esc_url(home_url('/my-account')) . '">', '</a>'); ?></p>
		<!-- /wp:paragraph -->
	</div>
	<!-- /wp:group -->
</div>
