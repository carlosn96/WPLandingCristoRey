<?php
/**
 * Port de renderer.py a PHP.
 * Genera el HTML editorial premium para Verbum Domini.
 */

function vbim_keyword_font_size($word)
{
    $n = strlen(trim($word));
    if ($n <= 6) {
...
function vbim_format_date_es($date_str)
{
    $months = array(
        "01" => "ENERO",
        "02" => "FEBRERO",
...
function vbim_render_editorial_html($keyword, $central_phrase, $passage_text, $image_url, $date_str = null, $citation = null)
{
    $keyword_font_size = vbim_keyword_font_size($keyword);
    $date_display = vbim_format_date_es($date_str);
    ...
    // Nota: El PHP no incluye las "Lecturas Relacionadas" porque estas se renderizan en el server side de WP idealmente o se manejan de forma dinámica.
    // Para ser EXACTAMENTE igual al pipeline, las omitimos o las dejamos vacías si no tenemos la lista.
    $related_posts_html = "";
    ...
    <style>
        ...
        .vb-entry-title {
            font-family: 'Cinzel', serif;
            font-weight: 700;
            font-size: <?php echo $keyword_font_size; ?>;
            line-height: 1.1;
